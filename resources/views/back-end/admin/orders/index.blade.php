											...
											<div class="wt-freelancers-content">
												<div class="dc-title">
													...
													<a href="{{{url('profile/'.$user->slug)}}}">
														<h3>{{{Helper::getUserName($user->id)}}}</h3>
													</a>
												</div>
											</div>
										</div>
								...
								<td>
									<span class="bt-content wt-payment-tab">
										<div class="wt-actionbtn">
											...
											@php
												$verify_package = \App\PackageTransaction::where("order_id",$order->id)->where("user_id",$order->user_id)->first();
											@endphp

											<!-- Button trigger modal -->
											<button type="button" class="wt-viewinfo wt-btnhistory text-white" data-toggle="modal" data-target="#orderModal-{{ $order->id }}">
												Payment Details ( {{ $verify_package->is_verified ? 'Verified' : 'Not Verified' }} )
											</button>
												...
												<div class="modal-dialog" role="document">
													<div class="modal-content">
														...
														<div class="modal-body">
															<p>
																<b>Payment Type</b> : {{ $order->package_transaction[0]->type }}
															</p>
															<p>
																<b>Payer Phone</b> : {{ $order->package_transaction[0]->phone }}
															</p>
															<p>
																<b>Transaction ID</b> : {{ $order->package_transaction[0]->transaction_id }}
															</p>
															@if( $verify_package->is_verified == 0)
															<form method="post" action="{{route('package.payment.approve', $verify_package->id)}}">
																@csrf

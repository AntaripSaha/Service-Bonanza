...
class UserController extends Controller
{
    ...
    public function submitReview(Request $request)
    {
        ...
        $service_user = ServiceUser::where("user_id", Auth::user()->id)->where("service_id", $request->service_id)->first();
        if ($service_user) {
            $service_user->status = 'completed';
            $service_user->paid = 'completed';
            $service_user->paid_progress = 'completed';
            $service_user->save();
        }
    ...
    public function checkout($id)
    {
        if (!empty($id)) {
            ...
            if (file_exists(resource_path('views/extend/back-end/package/checkout.blade.php'))) {
                return view::make('extend.back-end.package.checkout', compact('stripe_img', 'package', 'package_options', 'payment_gateway', 'symbol', 'mode', 'number', 'enable_package'));
            } else {
                return view::make('back-end.package.checkout', compact('stripe_img', 'package', 'package_options', 'payment_gateway', 'symbol', 'mode', 'number', 'enable_package'));
            }
        }
    }


    //make payment function start
    public function makePayment($id, Request $request)
    {

        $request->validate([
            'phone' => 'required',
            'transaction_id' => 'required|unique:package_transactions,transaction_id'
        ]);
        ...
        if ($package) {
            ...
            if ($invoice->save()) {
                ...
                //     $item = DB::table('items')->select('id')->where('subscriber', Auth::user()->id)->first();
                //     if (!empty($item)) {
                ...
                $item->product_id = $package->id;
                $item->subscriber = Auth::user()->id;
                $item->item_name = $package->title;
                $item->item_price = $package->cost;
                $item->item_qty = 1;
                $item->save();
                ...
                if ($order->save()) {
                    ...
                    $package_transaction->transaction_id = $request->transaction_id;
                    $package_transaction->type = "Bkash";

                    if ($package_transaction->save()) {
                        return redirect()->route('user.package', $package->id)->with('message', 'Package Purchased');
                    } else {
                        return back();
                    }
                } else {
    ...
    {
        $verify_package = PackageTransaction::orderBy('id', 'DESC')->get();
        $orders = Order::orderBy('id', 'DESC')->get();
        $currency   = SiteManagement::getMetaValue('commision');
        $symbol = !empty($currency) && !empty($currency[0]['currency']) ? Helper::currencyList($currency[0]['currency']) : array();
        $status_list = Helper::getOrderStatus();
        if (file_exists(resource_path('views/extend/back-end/admin/orders/index.blade.php'))) {
            return view::make('extend.back-end.admin.orders.index', compact('orders', 'symbol', 'status_list', 'verify_package'));
        } else {
            return view::make('back-end.admin.orders.index', compact('orders', 'symbol', 'status_list', 'verify_package'));
        }
    }
    //package verification start
    public function paymentApproved($id)
    {
        $payment = PackageTransaction::Where('id', '=', ($id))->first();
    ...
    public function getPayouts(Request $request)
    {
        ...
        // $payment_info = Payout::All();
        // return $payment_method = $payment_info->payment_method;

...
class FreelancerController extends Controller
{
    ...
    public function showOnGoingJobDetail($slug)
    {
        ...
        if (Auth::user()) {
            ...
            $job_transactions = JobTransaction::where("job_id", $job->id)->select("user_id", "transaction_id", "phone", "created_at", "type", "is_verified")->get();
    ...
    public function freelancerDashboard()
    {
        if (Auth::user()) {
            ...
            $package_item = Item::orderBy("id", "desc")->where('subscriber', $freelancer_id)->first();
     ...
     * @return \Illuminate\Http\Response
     */
    public function showServiceDetail($id, $status, $user_id)
    {
        if (Auth::user()) {
            ...
            $purchaser = ServiceUser::where("id", $id)->where("user_id", $user_id)->first();

...
class EmployerController extends Controller
{
    ...
    public function employerDashboard()
    {
        if (Auth::user()) {
            ...
            $package_item = Item::orderBy("id", "desc")->where('subscriber', $employer_id)->first();
            ...
            foreach ($all_packages as $all_package) {
                if (PackageTransaction::where("item_id", $all_package->id)->where("is_verified", true)->first()) {
                    $feature_job += (int)unserialize($all_package->package->options)['featured_jobs'];
                    $jobs += (int)unserialize($all_package->package->options)['jobs'];
                }
            }
    ...
    public function showServiceDetail($service_id, $pivot_id, $status)
    {
        if (Auth::user()) {
            ...
            $verified = ServiceTransaction::where("service_id", $service_id)->where("service_user_id", $pivot_id)->where("is_verified", true)->select("is_verified")->count();
            ...
            $service_transactions = ServiceTransaction::where("service_id", $service->id)->where("service_user_id", $pivot_id)->get();
    ...
    //hire freelancer function start
    public function hireFreelancer($id)
    {
        ...
        if ($proposal) {
            ...
            if ($job) {
                ...
                $job->status = 'hired';
                if ($job->save() && $proposal->save()) {
                    return redirect()->route('getProposals', $job->slug);
                }
            } else {
                abort(404);
            }
    ...
    {
        $request->validate([
            'transaction_id' => 'required|unique:job_trasnactions,transaction_id',
            'phone' => 'required|numeric|regex:/(01)[0-9]{9}/',
        ]);


        $job_transaction = new JobTransaction();
        $job_transaction->job_id = $request->job_id;
        $job_transaction->proposal_id = $request->proposal_id;
        $job_transaction->user_id = FacadesAuth::user()->id;
        $job_transaction->transaction_id  = $request->transaction_id;
        $job_transaction->phone  = $request->phone;
        $job_transaction->type  = "Bkash";

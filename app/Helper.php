...
class Helper extends Model
{
    ...
    public static function getFeatureJobs()
    {
        if (Auth::user()) {
            return Auth::user()->jobs->where('status','posted')->where("is_featured",'true')->count();
        }
    }
    ...
    public static function getFreelancerServices($status, $user_id, $paid_status = '')
    {
        $services = DB::table('services')
            ...
            ->select('services.*', 'service_user.id as pivot_id', 'service_user.user_id as pivot_user','service_user.seller_id as pivot_seller', 'service_user.type', 'service_user.status as pivot_status')
